...
//        List<String> firstOnes = bingoPool.subList(0, 15);
        ...
        var tempStream = bingoPool.stream() // stream pipeline start
                .limit(15)
                .filter(s -> s.indexOf('G') == 0 || s.indexOf("O") == 0)
                .map(s -> s.charAt(0) + "-" + s.substring(1))
                .sorted();
//                .forEach(s -> System.out.print(s + " ")); // terminal operation
        // stream은 원본 컬렉션에 영향을 주지 않음
        tempStream.forEach(s -> System.out.print(s + " "));
        // stream은 terminal operation으로 완성시키기 전까진 수행되지 않음
        System.out.println("\n--------------------------");
//        tempStream.forEach(s -> System.out.print(s.toLowerCase() + " "));
        // 한번 실행된 pipeline은 닫혀서 재사용이 불가능, 새로운 파이프라인을 생성해야함
